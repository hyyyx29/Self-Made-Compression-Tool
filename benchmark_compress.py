...
def file_compression_test(file: Path, temp_dir: Path):
    """Run compression tests on the given file
    using the given temporary directory to store
    the compressed and decompressed files.
    """
    ...
    compress.compress_file(str(file), str(temp_file_path) + ".huf")
    ...
    compressed_size = Path(str(temp_file_path) + ".huf").stat().st_size
    start_time = time.perf_counter()
    compress.decompress_file(str(temp_file_path) + ".huf", str(temp_file_path))
    stop_time = time.perf_counter()
    ...
    if not filecmp.cmp(file, temp_file_path):
        raise RuntimeError(
            f"File {file.name} was not compressed or decompressed correctly."
        )


def run_compression_tests(file_folder: Path):
    """Run compression tests on all files in the given folder."""
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for file in files:
            file_compression_test(file, temp_dir)
            print()
...
if __name__ == "__main__":
    ...
    run_compression_tests(file_folder)
